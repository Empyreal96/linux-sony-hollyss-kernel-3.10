...
*/
//#define BMC050_BLOCK_DAEMON_ON_SUSPEND //SW-PRODUCTION-JH-DMS07722368-CTS-
...
BMM050_RETURN_FUNCTION_TYPE bmm050api_init(struct bmm050api *bmm050)
{
	...
	//SW-PRODUCTION-JH-DMS07722368-CTS*[
	//bmm050api_set_presetmode(BMM050_PRESETMODE_REGULAR);
	bmm050api_set_presetmode(BMM050_PRESETMODE_HIGHACCURACY);
	//SW-PRODUCTION-JH-DMS07722368-CTS*]
	return comres;
}
...
static int bmm_i2c_read_block(struct i2c_client *client,
...
static int bmm_i2c_write_block(struct i2c_client *client, u8 addr,
			u8 *data, u8 len)
{
#ifdef CONFIG_I2C_BASIC_FUNCTION
	/*
...
{
	char strbuf[256];
	sprintf(strbuf, "bmc056d\n");
	return sprintf(buf, "%s", strbuf);		
}


/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(chipid,      S_IRUGO|S_IWUSR|S_IWGRP, show_chip_id, NULL);
...
static DRIVER_ATTR(cpsreptxy,      S_IRUGO|S_IWUSR|S_IWGRP, show_cpsreptxy_value, store_cpsreptxy_value);
static DRIVER_ATTR(cpsreptz,      S_IRUGO|S_IWUSR|S_IWGRP, show_cpsreptz_value, store_cpsreptz_value);
static DRIVER_ATTR(test,      S_IRUGO|S_IWUSR|S_IWGRP, show_test_value, store_test_value);
...
static int bmm050_wakeup(struct i2c_client *client)
{
	...
	while (try_times) {
		...
		mdelay(BMM_I2C_WRITE_DELAY_TIME);
		dummy = 0;
		err = bmm_i2c_read_block(client, BMM050_POWER_CNTL, &dummy, 1);
		if (data[1] == dummy)
		{
			break;
		}
...
/*----------------------------------------------------------------------------*/
#define BMM_MAX_RETRY_CHECKCHIPID (5)
static int bmm050_checkchipid(struct i2c_client *client)
{
	int err = 0;
	int try_times = BMM_MAX_RETRY_CHECKCHIPID;
	u8 chip_id = 0;

	while (try_times) {
		bmm_i2c_read_block(client, BMM050_CHIP_ID, &chip_id, 1);

		if ((chip_id & 0xff) != SENSOR_CHIP_ID_BMM)
		{
			MSE_ERR("tried count = %d\n", BMM_MAX_RETRY_WAKEUP - try_times + 1);
			try_times--;
		}
		else
		{
        ...
        MSE_ERR("read chip id result: %s, tried times: %d, chip_id: %#x\n",
                        (try_times > 0) ? "succeed" : "fail",
                        BMM_MAX_RETRY_WAKEUP - try_times + 1, chip_id);
...
{
	return bmm_i2c_read_block(this_client, reg_addr, data, len);
}
/*----------------------------------------------------------------------------*/
...
static int bmm050_init_client(struct i2c_client *client)
{
	...
	if (res < 0)
	{
		MSE_ERR("fail to bmm050_wakeup\n");
		return res;
	}
	res = bmm050_checkchipid(client); 
	if(res < 0)
	{
		MSE_ERR("fail to bmm050_checkchipid\n");
		return res;
	}	
	...
	if (res) 
	{
		MSE_ERR("fail to bmm050api_set_functional_state\n");
		return -EIO;
	}
...
static int bmm050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->client = client;
	data->client->timing = 400;//I2C timing set to 400
