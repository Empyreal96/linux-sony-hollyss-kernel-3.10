...
//CORE-KH-DbgCfgTool-UARTSwitch-00-a[
...
static int mtk_uart_startup(struct uart_port *port)
{
    ...
    if(!debug_uartmsg_enable)
    {
        MSG(ERR,"DO NOT enable uart interrupt since uartmsg disabled.\n");
    }
    else
    {
        MSG(ERR,"enable uart interrupt.\n");
...
//CORE-KH-DbgCfgTool-UARTSwitch-00-a[
#if defined(CONFIG_FIH_REMOVE_SERIAL_DYNAMICALLY)
	pr_info("UART init start ......\n");
	pr_info("debug_uartmsg_enable = 0x%x\n", debug_uartmsg_enable);
	if(!debug_uartmsg_enable)
	{
