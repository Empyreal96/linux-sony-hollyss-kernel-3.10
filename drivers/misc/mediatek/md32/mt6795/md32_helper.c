...
int get_md32_semaphore(int flag)
{
	...
	if (read_back == 0) {
		...
		pr_debug("[MD32] try to double get md32 semaphore %d\n", flag);
	}
...
int release_md32_semaphore(int flag)
{
	...
	if (read_back == 1) {
		...
		pr_debug("[MD32] try to double release md32 semaphore %d\n", flag);
	}
...
int md32_tcm_swap(int group_id)
{
	...
	if (swap_lock == 1) {
		...
		memcpy_md32((void *)swap_buf[g_current_group],
			    (const void *)(MD32_DTCM + g_group_info.app_d_area), swap_out_data_sz);
...
int reboot_load_md32(void)
{
	...
	int d_sz;
	int p_sz;
	unsigned char *md32_data_image_malloc;
	...
	md32_data_image_malloc = krealloc(md32_data_image, (size_t) d_sz + 1, GFP_KERNEL);
	md32_data_image = md32_data_image_malloc;
	if (!md32_data_image) {
...
int md32_dt_init(void)
{
	...
	pr_debug("[MD32] md32 dtcm base=0x%p\n", mreg->dtcm);

	if (!mreg->dtcm) {
		pr_debug("[MD32] Unable to ioremap dtcm\n");
		return -1;
	}
...
static int __init md32_init(void)
{
	...
	if (ret)
		pr_debug("[MD32] failed to request_irq for IPC_MD2HOST %d\n", ret);
