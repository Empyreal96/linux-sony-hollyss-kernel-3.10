...
static kal_bool is_ta_pattern = KAL_FALSE;
...
static void ich_pre_init(void)
{
	...
	if (is_ta_pattern == KAL_TRUE) {
		battery_xlog_printk(BAT_LOG_CRTI,"[ich_pre_init] bypass set ichr , Reg[0x%x]=0x%x\n", 0x8078, upmu_get_reg_value(0x8078));
		return;
	}
#endif
        //set ICH
    ...
    battery_log(BAT_LOG_TRK,"[ich_pre_init] Reg[0x%x]=0x%x\n", 0x8078, upmu_get_reg_value(0x8078));
}
...
static kal_uint32 is_chr_det(void)
{
    ...
    battery_log(BAT_LOG_TRK,"[is_chr_det] %d\n", val);
...
/* CORE-EL-fix_main-00+ */
extern bool ext_is_rechare_by_real_soc(void);
...
static void check_sw_recharge(void)
{
    ...
    g_sw_rechr_vlot=4600-(50*cv_val) - ext_get_resume_volt_delta();
	// CORE-EL-EXT_CHG_ALG-00*]
    ...
    if( (g_chr_complete_timer>=30) && (g_sw_rechr_flag==1) )
    {
		...
		if (ext_is_rechare_by_real_soc()) { // step 1: Check if soc is low...
			recharge = true;
		}
		else { // step 2: Else check if voltage is low
        ...
        if (recharge) {
            ...
            pmic_config_interface(0x8D30, 0x0, 0x1, 10);
            pmic_config_interface(0x8D34, 0x0, 0x1, 10);        
			battery_xlog_printk(BAT_LOG_CRTI, "recharge now!\n");
        }
    }
	/* CORE-EL-fix_main-00*] */
    ...
    battery_log(BAT_LOG_DBG,"[check_sw_recharge] volt_vbat=%d, g_sw_rechr_vlot=%d, g_chr_complete_timer=%d, g_sw_rechr_flag=%d\n",
...
void swchr_hw_init(void)
{
    ...
    //Reg[0x803A]
    mt6332_upmu_set_rg_iterm_sel(0x2);            //[15:13] ==> set iterm to 125mA
    ...
    if(upmu_is_chr_det()==1) //sync USB device/otg state
    {
        ...
        else
        {
            ...
            if( (ich_low_val==1) || (is_charge_complete==1) )
            {
                ...
                {
                    /* CORE-EL-wrkunplugadaptor-00+[ */
                    kal_int32 chr_volt = battery_meter_get_charger_voltage();
                    msleep(10);
                ...
                {
                    battery_log(BAT_LOG_DBG,"[chr_plug_out_sw_detect] nothing 1\n");
                }
            }
            else
            {
                set_usb_dc_in_mode(0,0);
                battery_log(BAT_LOG_DBG,"[chr_plug_out_sw_detect] nothing 2\n");
            }
#ifdef CONFIG_MTK_ENABLE_PP
            //------------------------------------------------
...
static kal_uint32 charging_get_charger_det_status(void *data)
{
    ...
    *(kal_bool*)(data) = val;
    battery_log(BAT_LOG_TRK,"CHRDET=%d\n", val);
