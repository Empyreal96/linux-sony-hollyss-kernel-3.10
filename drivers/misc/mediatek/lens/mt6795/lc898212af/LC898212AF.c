...
void LC898212AF_init_drv(void)
{
	...
	ReadEEPROM(0x02E4, &posl);		   
	ReadEEPROM(0x02E5, &posh);  
	min_pos =  (posh<<8) + posl;
    ReadEEPROM(0x02E6, &posl);		   
    ReadEEPROM(0x02E7, &posh);
    max_pos =  (posh<<8) + posl;
	
	ReadEEPROM(0x04A9, &eepdata1);		   
	No_eeprom = ReadEEPROM(0x04A8, &eepdata2);
	...
	/*MM-SL-AddIMX230EEPROM-01*{ */
    if(No_eeprom == 0 && eepdata2!= 0xff)
    {
    ...
    {
        WriteI2C(0,0x28,0xF0); //Ofset 
        WriteI2C(0,0x29,0xAF); //Bias
    }
	/*MM-SL-AddIMX230EEPROM-01*} */
    ...
    WriteI2C(1, 0x40, 0x4030); //0x8010
    WriteI2C(1, 0x42, 0x7150); //0x7570
    WriteI2C(1, 0x44, 0x8F90); //0x8b50
    WriteI2C(1, 0x46, 0x61B0); //0x6a10
    WriteI2C(1, 0x48, 0x7FF0); //0x5a90
    WriteI2C(1, 0x4a, 0x3930); //0x2030
    WriteI2C(1, 0x4c, 0x4030); //0x32f0
    WriteI2C(1, 0x4e, 0x8010); //0x7ff0
    WriteI2C(1, 0x50, 0x04f0);
    WriteI2C(1, 0x52, 0x7610);
    WriteI2C(1, 0x54, 0x2030); //0x1450
...
/*MM-SL-ModifyLC898212AFSetting-00*} */

/*MM-SL-FixCantFocusForFarDistance-00*{ */
int SetVCMPos(u16 _wData)
{
    ...
    if( _wData >= 512 ) 
		TargetPos =  ((_wData - 512)<< 6)&0xFFFF; 
    else                 
		TargetPos = ((_wData << 6) + 0x8000)&0xFFFF;

	//LOG_INF("SetVCMPos:_wData = 0x%x, TargetPos = 0x%x\n", _wData, TargetPos);
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (g_s4AF_Opened == 1) {
        ...
        if(ret == 0)
        {
        	/*MM-SL-FixItsNotFocusWhenOpenCamera-00+{ */
        	if (InitPos > 0)
				InitPos = 0;
			/*MM-SL-FixItsNotFocusWhenOpenCamera-00+} */
            LOG_INF("Init Pos %6d \n", InitPos);
