	...
	if (prBssDesc) {
		if (prBssDesc->eChannelWidth == CW_20_40MHZ) {
			if ((prBssDesc->eSco == CHNL_EXT_SCA) || (prBssDesc->eSco == CHNL_EXT_SCB)) {
				...
				printk(KERN_INFO "%s:CW_20_40MHZ ucAPBandwidth = MAX_BW_40MHZ\n", __func__);
			}
			else {
				ucAPBandwidth = MAX_BW_20MHZ;
				printk(KERN_INFO "%s:CW_20_40MHZ ucAPBandwidth = MAX_BW_20MHZ\n", __func__);
			}
		}
#if (CFG_FORCE_USE_20BW == 1)
		if (prBssDesc->eBand == BAND_2G4) {
			ucAPBandwidth = MAX_BW_20MHZ;
			printk(KERN_INFO "%s:BAND_2G4 ucAPBandwidth = MAX_BW_20MHZ\n", __func__);
		}
#endif
	}

	printk(KERN_INFO "%s: leave ucAPBandwidth = %d\n", __func__, ucAPBandwidth);
