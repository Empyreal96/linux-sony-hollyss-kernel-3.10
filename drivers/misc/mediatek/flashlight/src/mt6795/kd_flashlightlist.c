...
/* MM-MC-FixFlashLedTurnOnIssue-00+{ */
bool gIsFlashFirstOpen = true;
extern int fl_first_init(void); 
/* MM-MC-FixFlashLedTurnOnIssue-00+} */
...
static int flashlight_open(struct inode *inode, struct file *file)
{
    ...
    /* MM-MC-FixFlashLedTurnOnIssue-00+{ */
    logI("[flashlight_open] gIsFlashFirstOpen = %d", gIsFlashFirstOpen);
    if (gIsFlashFirstOpen)
    {
        fl_first_init();
        gIsFlashFirstOpen = false;
    }
    /* MM-MC-FixFlashLedTurnOnIssue-00+} */
    return i4RetValue;
}
