...
static int __init ram_console_save_old(struct ram_console_buffer *buffer, size_t buffer_size)
{
	...
	memcpy(ram_console_old, buffer, buffer_size);
        //[somc][ramdump]ram console may be corrupted after this memcpy in case of cold boot.
	//so add the following memcpy.
	memcpy(buffer, ram_console_old, buffer_size);
	return 0;
}
...
static int __init ram_console_late_init(void)
{
	...
	if (ram_console_check_header(ram_console_old)) {
		pr_err("ram_console: invalid buffer, skip /proc/last_kmsg\n");
		return 0;
	}
