...
static bool PanelState = 0x1;/* MM-GL-DISPLAY-panel-12+ */
...
/* PERI-AH-Disable HW control-00+[ */
#define LED_GPIO 40
bool disabled_hw_control = false;
void fxn_disable_hw_control(unsigned int pmic_type, unsigned int level)
{
        int ret;

        if (disabled_hw_control) {
        return;
        }

        if ((pmic_type == MT65XX_LED_PMIC_NLED_ISINK0 ||
      pmic_type == MT65XX_LED_PMIC_NLED_ISINK1 ||
      pmic_type == MT65XX_LED_PMIC_NLED_ISINK2) && level) {

            ret = mt_set_gpio_mode(LED_GPIO, GPIO_MODE_00);
            if (0 != ret) {
                LEDS_DEBUG("Driver_LED mt_set_gpio_mode fail\n");
            return;
            }

        ret = mt_set_gpio_dir(LED_GPIO, GPIO_DIR_OUT);
            if (0 != ret) {
                LEDS_DEBUG("Driver_LED mt_set_gpio_dir fail\n");
            return;
            }

        ret = mt_set_gpio_out(LED_GPIO, 1);
            if (0 != ret) {
                LEDS_DEBUG("Driver_LED mt_set_gpio_out fail\n");
            return;
            }

        ret = mt_set_gpio_pull_enable(LED_GPIO, GPIO_PULL_ENABLE);
            if (0 != ret) {
                LEDS_DEBUG("Driver_LED mt_set_gpio_pull_enable fail\n");
            return;
            }

                ret = mt_set_gpio_pull_select(LED_GPIO, GPIO_PULL_UP);
            if (0 != ret) {
                LEDS_DEBUG("Driver_LED mt_set_gpio_pull_select fail\n");
            return;
            }

                disabled_hw_control = true;
        LEDS_DEBUG("Driver_LED disable HW control LED completed. disabled_hw_control:%d \n", disabled_hw_control);
        }
}
/* PERI-AH-Disable HW control-00+] */
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_LCD_ISINK) {
		...
		mt6331_upmu_set_isink_ch0_mode(PMIC_PWM_0);
		mt6331_upmu_set_isink_ch0_step(ISINK_3);	/* 16mA */
		/* PERI-AH-Change to use brightness file node dynamic setting
                mt6331_upmu_set_isink_dim0_duty(15);
        */
		mt6331_upmu_set_isink_dim0_fsel(ISINK_1KHZ);	/* 1KHz */
		if (level) {
                        /* PERI-AH-use brightness file node dynamic setting-00+[ */
                        if((level > 0) && (level <= 255))
                        {
                                level = level / 8;
                                mt6331_upmu_set_isink_dim0_duty(level);
                        }
                        /* PERI-AH-use brightness file node dynamic setting-00+] */
			mt6331_upmu_set_isink_ch0_en(NLED_ON);
		} else {
		...
		if (level) 
		{
                        ...
                        {
                                level = level / 8;
                                mt6331_upmu_set_isink_dim1_duty(level);
                        }
                        /* PERI-AH-use brightness file node dynamic setting-00+] */
		...
		if (level) {
                        ...
                        {
                                level = level / 8;
                                mt6331_upmu_set_isink_dim2_duty(level);
                        }
                        /* PERI-AH-use brightness file node dynamic setting-00+] */
			mt6331_upmu_set_isink_ch2_en(NLED_ON);
		} else {
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	unsigned int BacklightLevelSupport = Cust_GetBacklightLevelSupport_byPWM();
	int ret; /* PERI-AH-Disable HW control-00+ */
	...
	case MT65XX_LED_MODE_PMIC:
        ...
        if (ret == 0) {
                        fxn_disable_hw_control(cust->data, level);
                }
        return ret;
        /* PERI-AH-Disable HW control-00+] */
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	/* MM-GL-DISPLAY-panel-16+[ */
	int bl_lvl;
	bl_lvl = bkl_lut[level];
	/* MM-GL-DISPLAY-panel-16+] */
#ifdef CONFIG_MTK_AAL_SUPPORT
	/* MM-GL-DISPLAY-panel-16+ */
	//if (led_data->level != level) {
	if(led_data->level != bl_lvl){

	//	led_data->level = level;
	led_data->level = bl_lvl;
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
			...
			//				    1) * level + 127) / 255);			
			disp_aal_notify_backlight_changed( (835*bl_lvl + 127)/255 );/* MM-GL-DISPLAY-panel-16+ */
		}
	}
	/* MM-GL-DISPLAY-panel-16+ */
#else
	...
	if (led_data->level != level) {
		...
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
			...
			//				  1) * level + 127) / 255));
			/* MM-GL-DISPLAY-panel-15-
				mt_mt65xx_led_set_cust(&led_data->cust, ((835*bl_lvl + 127)/255));/* MM-GL-DISPLAY-panel-16+ */
			} else {
				mt_mt65xx_led_set_cust(&led_data->cust, led_data->level);
			}
		}
...
#define PMIC_BLINKING_CYCLE     6099
#define PMIC_BLINKING_ON_DUTY 0
#define PMIC_MAX_BRIGHENESS     255
#define PMIC_MIN_CURRENT        4
#define PMIC_MAX_CURRENT        24

//4, 8, 12, 16, 20, 24mA
int pmic_current_array[] = { ISINK_0, ISINK_1, ISINK_2, ISINK_3, ISINK_4, ISINK_5 };


void fxn_mt65xx_set_brightness(unsigned int param1, unsigned int param2, unsigned int param3)
{
    LED_MSG("%d %d %d", param1, param2, param3);

    mutex_lock(&leds_pmic_mutex);

    if (param1 == 0 && param2 == 0 && param3 == 0) {
        LED_MSG("turn off all off leds ");
                mt6331_upmu_set_isink_ch0_en(NLED_OFF);
                mt6331_upmu_set_isink_ch1_en(NLED_OFF);
                mt6331_upmu_set_isink_ch2_en(NLED_OFF);
        mutex_unlock(&leds_pmic_mutex);
    ...
    mt6331_upmu_set_rg_drv_32k_ck_pdn(0x0); // Disable power down
        //red
    if((param1 > 0) && (param1 <= PMIC_MAX_BRIGHENESS))
    {
        param1 = param1 / 8;
        mt6331_upmu_set_isink_dim0_duty(param1);
        LED_MSG("set red brightness %d", param1);
    }

        //green
    if((param2 > 0) && (param2 <= PMIC_MAX_BRIGHENESS))
    {
        param2 = param2 / 8;
        mt6331_upmu_set_isink_dim1_duty(param2);
        LED_MSG("set green brightness %d", param2);
    }

    //blue
    if((param3 > 0) && (param3 <= PMIC_MAX_BRIGHENESS))
    {
        param3 = param3 / 8;
        mt6331_upmu_set_isink_dim2_duty(param3);
        LED_MSG("set blue brightness %d", param3);
    }

        mutex_unlock(&leds_pmic_mutex);
    LED_MSG("set brightness completed");


}

void fxn_mt65xx_set_current(unsigned int param1, unsigned int param2, unsigned int param3)
{
    LED_MSG("%d %d %d", param1, param2, param3);
    ...
    mt6331_upmu_set_rg_drv_32k_ck_pdn(0x0); // Disable power down
        ...
        //red
    if((param1 >= PMIC_MIN_CURRENT) && (param1 <= PMIC_MAX_CURRENT))
    {
        param1 = (param1 / 4)-1;
        mt6331_upmu_set_isink_ch0_step(pmic_current_array[param1]);
        LED_MSG("set red current step %d", param1);
    }

        //green
    if((param2 >= PMIC_MIN_CURRENT) && (param2 <= PMIC_MAX_CURRENT))
    {
        param2 = (param2 / 4)-1;
        mt6331_upmu_set_isink_ch1_step(pmic_current_array[param2]);
        LED_MSG("set green current step %d", param2);
    }

    //blue
    if((param3 >= PMIC_MIN_CURRENT) && (param3 <= PMIC_MAX_CURRENT))
    {
        param3 = (param3 / 4)-1;
        mt6331_upmu_set_isink_ch2_step(pmic_current_array[param3]);
        LED_MSG("set blue current step %d", param3);
    }

        mutex_unlock(&leds_pmic_mutex);
    LED_MSG("set current completed");

}

void fxn_mt65xx_set_led(unsigned int param1, unsigned int param2, unsigned int param3)
{
    LED_MSG("%d %d %d", param1, param2, param3);
    ...
    if (param1 == NLED_OFF && param2 == NLED_OFF && param3 == NLED_OFF) {
        LED_MSG("turn off all off leds ");
        mutex_unlock(&leds_pmic_mutex);
                return;
        }
    ...
    mt6331_upmu_set_isink_ch0_en(param1);
        mt6331_upmu_set_isink_ch1_en(param2);
        mt6331_upmu_set_isink_ch2_en(param3);

        mutex_unlock(&leds_pmic_mutex);
    LED_MSG("set led completed");

        fxn_disable_hw_control(MT65XX_LED_PMIC_NLED_ISINK0, NLED_ON);

}

void fxn_mt65xx_set_blink(unsigned int param1, unsigned int param2, unsigned int param3)
{
    ...
    if (param1 < PMIC_MIN_CURRENT && param2 < PMIC_MIN_CURRENT && param3 < PMIC_MIN_CURRENT) {
        LED_MSG("turn off all off leds blink");
        mutex_unlock(&leds_pmic_mutex);
        return;
        }

    mt6331_upmu_set_rg_drv_32k_ck_pdn(0x0); // Disable power down
    //blinking set on 0.19s/off 6s
        if (param1 >= PMIC_MIN_CURRENT) {
        LED_MSG("set red blinking parameter");
            mt6331_upmu_set_rg_drv_isink0_ck_pdn(0);
            mt6331_upmu_set_rg_drv_isink0_ck_cksel(0);
            mt6331_upmu_set_isink_ch0_mode(PMIC_PWM_0);
        param1 = (param1 / 4)-1;
        mt6331_upmu_set_isink_ch0_step(pmic_current_array[param1]);
            mt6331_upmu_set_isink_dim0_duty(PMIC_BLINKING_ON_DUTY);
            mt6331_upmu_set_isink_dim0_fsel(PMIC_BLINKING_CYCLE);
        param1 = NLED_ON;
        }
    else {
        param1 = NLED_OFF;
        ...
        if (param2 >= PMIC_MIN_CURRENT) {
        LED_MSG("set green blinking parameter");
            mt6331_upmu_set_rg_drv_isink1_ck_pdn(0);
            mt6331_upmu_set_rg_drv_isink1_ck_cksel(0);
            mt6331_upmu_set_isink_ch1_mode(PMIC_PWM_0);
        param2 = (param2 / 4)-1;
            mt6331_upmu_set_isink_ch1_step(pmic_current_array[param2]);
            mt6331_upmu_set_isink_dim1_duty(PMIC_BLINKING_ON_DUTY);
            mt6331_upmu_set_isink_dim1_fsel(PMIC_BLINKING_CYCLE);
        param2 = NLED_ON;
    }
    else {
        param2 = NLED_OFF;
        ...
        if (param3 >= PMIC_MIN_CURRENT) {
        LED_MSG("set blue blinking parameter");
            mt6331_upmu_set_rg_drv_isink2_ck_pdn(0);
            mt6331_upmu_set_rg_drv_isink2_ck_cksel(0);
            mt6331_upmu_set_isink_ch2_mode(PMIC_PWM_0);
        param3 = (param3 / 4)-1;
            mt6331_upmu_set_isink_ch2_step(pmic_current_array[param3]);
            mt6331_upmu_set_isink_dim2_duty(PMIC_BLINKING_ON_DUTY);
            mt6331_upmu_set_isink_dim2_fsel(PMIC_BLINKING_CYCLE);
        param3 = NLED_ON;
        }
    else {
        param3 = NLED_OFF;
    ...
    LED_MSG("set blinking completed");
        mutex_unlock(&leds_pmic_mutex);
}
/* PERI-AH-Add control state-00+] */
