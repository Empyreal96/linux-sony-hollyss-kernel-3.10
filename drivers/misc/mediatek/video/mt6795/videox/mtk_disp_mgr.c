...
int disp_get_session_number(void)
{
	...
	for (i = 0; i < MAX_SESSION_COUNT; i++) {
		if (session_config[i] != 0) {
			DISPMSG("list session: [%d] 0x%x\n", session_cnt, session_config[i]);
			session_cnt++;
		}
	}
...
int disp_destroy_session(disp_session_config *config)
{
	...
	for (i = 0; i < MAX_SESSION_COUNT; i++) {
		if (session_config[i] == session) {
			session_config[i] = 0;
			ret = 0;
			DISPMSG("Destroy session %d:0x%x\n", i, session);
			break;
		}
	}
...
int _ioctl_get_display_caps(unsigned long arg)
{
    ...
    //still need to be notified when VP start/end from GuiExt
...
int set_session_mode(disp_session_config *config_info, int force)
{
    ...
    static unsigned sStayInVP = 0;
    if (config_info->user == SESSION_USER_GUIEXT) {
        //TODO: start/end miravision path
        if (config_info->mode == DISP_SESSION_DECOUPLE_MODE) {
            // START miravision
            DISPMSG("[FB]: >>>>>MiraVision start\n");
			if (sStayInVP == 0 && (!ovl2mem_is_connected()
			#ifdef CONFIG_MTK_HDMI_SUPPORT
				|| !is_hdmi_active()
			#endif
				)) {
				DISPMSG("Session num: %d\n", disp_get_session_number());
				sStayInVP = 1;
				atomic_set(&isMiraPath, 1);
				primary_display_init_path_miravision();
            }

        } else if (config_info->mode == DISP_SESSION_DIRECT_LINK_MODE) {
            // END miravision
            DISPMSG("[FB]: <<<<<MiraVision end\n");
            if (sStayInVP == 1) {
                sStayInVP = 0;
                atomic_set(&isMiraPath, 0);
                primary_display_deinit_path_miravision();
            }
        }
