...
static int mtkfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case MTKFB_SET_OVERLAY_LAYER:
	{
		...
		if (copy_from_user(&layerInfo, (void __user *)arg, sizeof(layerInfo))) {
			...
			memset((void *)&input, 0, sizeof(primary_disp_input_config));
			if (layerInfo.layer_id >= HW_OVERLAY_COUNT) {
				disp_aee_print
...
static ssize_t disp_lcm_manufactory_id_show(struct device *dev,
               struct device_attribute *attr, char *buf)
{
       unsigned char manufactoryID = disp_lcm_manufacture_id_read();

       return snprintf(buf, PAGE_SIZE, "%x\n", manufactoryID);
}
/* MM-GL-DISPLAY-panel-02+] */
static DEVICE_ATTR(manufactory_id, 0644, disp_lcm_manufactory_id_show, NULL);/* MM-GL-DISPLAY-panel-02+ */
...
/* MM-GL-DISPLAY-panel-02+[ */
	if(fbi->node == 0)
	{
		...
		r = device_create_file(fbi->dev, &dev_attr_manufactory_id);
		if (r) {
