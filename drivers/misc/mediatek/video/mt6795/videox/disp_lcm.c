...
static unsigned char disp_lcm_manufacture_ID = 0;/* MM-GL-DISPLAY-panel-02+ */
/* these 2 variables are defined in mt65xx_lcm_list.c */
extern LCM_DRIVER *lcm_driver_list[];
extern unsigned int lcm_count;
/* MM-GL-DISPLAY-panel-02+[ */
unsigned int IS_LCM_INIT_READY;
unsigned int IS_LCM_POW_READY;
unsigned int lcm_first_suspend;/* MM-GL-DISPLAY-panel-08+ */
unsigned int IS_GET_LCM_ID;
unsigned int LCM_ID;
/* MM-GL-DISPLAY-panel-02+] */
...
disp_lcm_handle *disp_lcm_probe(char *plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	LCM_ID = 0xFF;
	/* MM-GL-DISPLAY-panel-02+] */
	lcm_first_suspend = 0;/* MM-GL-DISPLAY-panel-08+ */
		...
		if (plcm_name == NULL) {
			...
			for (i = 0; i < _lcm_count(); i++) {
				...
				if (!strcmp(lcm_drv->name, plcm_name)) {
					...
					{
						case 0:
							disp_lcm_manufacture_ID = 0x81;
						break;
						case 1:
							disp_lcm_manufacture_ID = 0x82;
						break;
						case 2:
						/* MM-GL-DISPLAY-panel-21+[ */
							disp_lcm_manufacture_ID = 0x91;
						break;
						/* MM-GL-DISPLAY-panel-21+] */
						default:
							disp_lcm_manufacture_ID = 0x00;
						break;
					}
					/* MM-GL-DISPLAY-panel-02+] */
					break;
			...
			/* MM-GL-DISPLAY-panel-15- *///DISPERR("FATAL ERROR: can't found lcm driver:%s in linux kernel driver\n", plcm_name);
			DISPCHECK("[DISPLAY]LCM panel ID=%d\n", disp_lcm_manufacture_ID);/* MM-GL-DISPLAY-panel-15+ */
		}
		/* TODO: */
	}
...
/* MM-GL-DISPLAY-panel-02+[ */
unsigned char disp_lcm_manufacture_id_read(void)
{
	return disp_lcm_manufacture_ID;
}
/* MM-GL-DISPLAY-panel-02+] */
...
/* MM-GL-DISPLAY-panel-24+[ */
int dis_lcm_power_resume(disp_lcm_handle *plcm)
{
	DISPFUNC();
	LCM_DRIVER *lcm_drv = NULL;
...
int disp_lcm_resume(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
#if 0/* MM-GL-DISPLAY-panel-24+ */
		if (lcm_drv->resume_power)
			lcm_drv->resume_power();

#endif/* MM-GL-DISPLAY-panel-24+] */
		if (lcm_drv->resume) {
			lcm_drv->resume();
		} else {
