...
int ovl2mem_init(unsigned int session)
{
    ...
    if (atomic_read(&isMiraPath)==1) {
        DISPMSG("Start memory session, shutdown MiraVision\n");
...
int ovl2mem_deinit(void)
{
    ...
    extern atomic_t isMiraPath;
    if (atomic_read(&isMiraPath)==1) {
        DISPMSG("Stop memory session, startup MiraVision\n");
...
int ovl2mem_is_connected(void)
{
	return (pgc->dpmgr_handle != NULL) ? 1 : 0;
}
