...
static kal_int32 car_wakeup = 0x12345678;
/* aging mechanism */
#ifdef MTK_ENABLE_AGING_ALGORITHM
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	if (vbat_val > VBAT_NORMAL_WAKEUP) {	/* 3.6v */
		...
		bm_print(BM_LOG_CRTI,
			 "[get_dynamic_period] car_wakeup=%d, car_sleep=%d, I_sleep=%d, gFG_BATT_CAPACITY=%d, pre_time=%d, add_time=%d, last_time=%d, new_time=%d , g_spm_timer=%d, battery_capacity_level = %d\r\n",
			 car_wakeup, car_sleep, I_sleep, gFG_BATT_CAPACITY_aging, pre_time, add_time, last_time,
			 new_time, g_spm_timer, battery_capacity_level);
		if (new_time > 1000)
			new_time = 1000;
		ret_val = new_time;
...
#if defined(SOC_BY_HW_FG)
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CAR_ACT, &car_wakeup);
#ifdef MTK_ENABLE_AGING_ALGORITHM
