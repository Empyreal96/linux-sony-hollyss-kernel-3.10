...
static u8 g_proximity_en = 0;
	...
	{		
		#include "SONY_Truly_S50_E1703_2015-11-12_CFG07_Release.i"
	};
#endif
...
static int irqtest_flag = 0;
static int irqtest_count = 0;
...
#ifdef HX_EN_GLOVE
static void himax_set_glove_func(unsigned int enable)
{
	uint8_t cmd[4];
...
{
	uint8_t buf[5];
	if (i2c_himax_read(ts->client, 0x86, buf, 4,HIMAX_I2C_RETRY_TIMES)<0)	
...
inline void himax_ts_work()
{
	...
	if (ret<0)
#else
	if (i2c_himax_read(ts->client, 0x86, buf, hx_touch_info_size,HIMAX_I2C_RETRY_TIMES)<0)
#endif
	{
		E("%s: can't read data from chip!\n", __func__);
...
#if defined(HX_EN_GLOVE)
	if(ts->glove_enable)
		glove_event = (buf[HX_TOUCH_INFO_POINT_CNT+2] & 0x04);
	else
		glove_event = 0x00;
#endif
						...
						if (((finger_pressed >> loop_i) & 1) == 1) {
							...
							//if ((ts->debug_log_level & BIT(3)) > 0)
							{
								if ((((old_finger >> loop_i) ^ (finger_pressed >> loop_i)) & 1) == 1)
								{
									if (ts->useScreenRes)
									{
										I("ETUCH : status:%X, Screen:F:%02d Down, X:%d, Y:%d, W:%d, N:%d Glove:%d\n",
										finger_pressed, loop_i+1, x * ts->widthFactor >> SHIFTBITS,
										y * ts->heightFactor >> SHIFTBITS, w, EN_NoiseFilter,glove_event);
									}
									else
									{
										I("ETUCH : status:%X, Raw:F:%02d Down, X:%d, Y:%d, W:%d, N:%d Glove:%d\n",
										finger_pressed, loop_i+1, x, y, w, EN_NoiseFilter,glove_event);
									}
								}
							}
							...
							if(glove_event)
							{
								input_report_abs(ts->input_dev, ABS_MT_PRESSURE, 255+5);
							}
							else
							{
							...
							//if ((ts->debug_log_level & BIT(3)) > 0)
							{
								if ((((old_finger >> loop_i) ^ (finger_pressed >> loop_i)) & 1) == 1)
								{
									if (ts->useScreenRes)
									{
										I("ETUCH : status:%X, Screen:F:%02d Up, X:%d, Y:%d, N:%d Glove:%d\n",
										finger_pressed, loop_i+1, ts->pre_finger_data[loop_i][0] * ts->widthFactor >> SHIFTBITS,
										ts->pre_finger_data[loop_i][1] * ts->heightFactor >> SHIFTBITS, Last_EN_NoiseFilter,glove_event);
									}
									else
									{
										I("ETUCH : status:%X, Raw:F:%02d Up, X:%d, Y:%d, N:%d Glove:%d\n",
										finger_pressed, loop_i+1, ts->pre_finger_data[loop_i][0],
										ts->pre_finger_data[loop_i][1], Last_EN_NoiseFilter,glove_event);
									}
								}
							}
						}
					...
					for (loop_i = 0; loop_i < ts->nFinger_support /*&& (ts->debug_log_level & BIT(3)) > 0*/; loop_i++) {
						if (((ts->pre_finger_mask >> loop_i) & 1) == 1) {
							if (ts->useScreenRes) {
								...
								I("ETUCH : status:%X, Raw:F:%02d Up, X:%d, Y:%d, N:%d Glove:%d\n",0, loop_i+1, ts->pre_finger_data[loop_i][0],ts->pre_finger_data[loop_i][1], Last_EN_NoiseFilter,glove_event);
							}
						}
					}
...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	/*INT mode*/
		ts->use_irq = 1;	
	touch_thread = kthread_run(touch_event_handler, private_ts, "himax-tpd");
...
static ssize_t himax_irqtest_show(struct device *dev,
               struct device_attribute *attr, char *buf)
{
       int value = irqtest_count ;
       irqtest_count = 0 ;
       irqtest_flag = 0 ;
       return snprintf(buf, PAGE_SIZE, "%d\n",value);
}

static ssize_t himax_irqtest_store(struct device *dev,
               struct device_attribute *attr, const char *buf, size_t count)
{
       irqtest_flag = 1;
       return count;
...
static DEVICE_ATTR(irqtest, (S_IWUSR|S_IRUGO),
	himax_irqtest_show, himax_irqtest_store);
...
static ssize_t himax_debug_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	else if ( buf[0] == 'c') //firmware version
	{
		debug_level_cmd = buf[0];
		himax_read_FW_ver(true);
		return count;
...
#ifdef HX_EN_GLOVE
static ssize_t himax_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct himax_ts_data *ts = private_ts;
	size_t count = 0;
...
static ssize_t himax_glove_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "0"))
		...
		return -EINVAL;
	himax_set_glove_func(ts->glove_enable);

	I("%s: glove_enable = %d.\n", __func__, ts->glove_enable);

	return count;
}

static DEVICE_ATTR(glove, (S_IWUSR|S_IRUGO|S_IWUGO),
	himax_glove_show, himax_glove_store);
#endif
...
static int himax_touch_sysfs_init(void)
{
	...
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_irqtest.attr);
	if (ret) {
		E("%s: sysfs_create_file dev_attr_layout failed\n", __func__);
		return ret;
	}
	...
	if (ret)
	{
		...
		if (ret) 
		{
			E("sysfs_create_file dev_attr_glove failed\n");
			return ret;
		}
	#endif
...
{
	pr_info("MTKTOUCH tpd_local_init driver \n");
	TPD_DMESG("Himax I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
...
static struct tpd_driver_t himax_driver = {
	.tpd_device_name = "Himax852xes",
...
struct himax_i2c_platform_data himax_852xes_ts_data = {
	.abs_x_min = 0,
	.abs_x_max = 1080,
	.abs_y_min = 0,
	.abs_y_max = 1920,
...
static struct i2c_board_info __initdata i2c_himax_tpd={
		I2C_BOARD_INFO("Himax852xes", (0x94>>1))
		//.platform_data  =(void *)&himax_852xes_ts_data,
};
...
{	
	...
	pdata->abs_x_min = 0,
	pdata->abs_x_max = 1080,
	pdata->abs_y_min = 0,
	pdata->abs_y_max = 1920,
	pdata->abs_pressure_min = 0,
	pdata->abs_pressure_max = 200,
	pdata->abs_width_min = 0,
	pdata->abs_width_max = 200,
	pdata->gpio_irq = 6,
	pdata->gpio_reset = 102,
	pdata->protocol_type = PROTOCOL_TYPE_B,
	pdata->screenWidth = 1080,
	pdata->screenHeight = 1920,
	i2c_himax_tpd.platform_data = pdata;
	i2c_register_board_info(TPD_I2C_BUS_ID, &i2c_himax_tpd, 1);
	
	if(tpd_driver_add(&himax_driver) < 0){
		pr_err("Fail to add himax tpd driver\n");
		return -1;
	}
