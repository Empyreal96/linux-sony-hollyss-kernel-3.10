...
show_prototype(fullrawdata)
show_prototype(diffcount)
show_prototype(trxshort)
show_store_prototype(cbc_control)
store_prototype(reset)
store_prototype(irq_enable)
...
struct device_attribute systemattrs[] = {
	__ATTR(irq_enable, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_show_error,
			synaptics_rmi4_f54_irq_enable_store),
	__ATTR(cbc_control, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_f54_cbc_control_show,
			synaptics_rmi4_f54_cbc_control_store),
};
...
static ssize_t synaptics_rmi4_f54_rawdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&f54->status_mutex);
	command = (unsigned char)0x01;
	retval = f54->fn_ptr->write(rmi4_data,0xFF,&command,sizeof(command));
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to change page\n",
				__func__);
		mutex_unlock(&f54->status_mutex);
		return -EINVAL;
	}
	...
	f54->report_size = f54->rx_assigned * f54->tx_assigned * 2;

	set_interrupt(true);
	
	command = (unsigned char)3;
	retval = f54->fn_ptr->write(rmi4_data,f54->data_base_addr,&command,sizeof(command));
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to set type\n",
				__func__);
		mutex_unlock(&f54->status_mutex);
		return -EINVAL;
	}
	...
	report_index[0] = 0;
	report_index[1] = 0;
	retval = f54->fn_ptr->write(rmi4_data,f54->data_base_addr + DATA_REPORT_INDEX_OFFSET,report_index,sizeof(report_index));
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to set node\n",
				__func__);
		mutex_unlock(&f54->status_mutex);
		return -EINVAL;
	}
	...
	retval = f54->fn_ptr->write(rmi4_data,f54->command_base_addr,&command,sizeof(command));
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to get report\n",
				__func__);
		mutex_unlock(&f54->status_mutex);
		return -EINVAL;
	}
	...
	do {
		mutex_lock(&f54->status_mutex);
		retval =f54->fn_ptr->read(rmi4_data,f54->command_base_addr,&command,sizeof(command));
		mutex_unlock(&f54->status_mutex);
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
					"%s: Failed to get report\n",
	...
	mutex_lock(&f54->status_mutex);
	retval = f54->fn_ptr->read(rmi4_data,f54->data_base_addr + DATA_REPORT_DATA_OFFSET,f54->report_data,f54->report_size);
	mutex_unlock(&f54->status_mutex);
	if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
					"%s: Failed to read raw data\n",
					__func__);
			return -EINVAL;
	}
...
static ssize_t synaptics_rmi4_f54_fullrawdata_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	int index = 0;
...
static ssize_t synaptics_rmi4_f54_diffcount_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	int index = 0;
...
static ssize_t synaptics_rmi4_f54_trxshort_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	int index = 0;
...
static ssize_t synaptics_rmi4_f54_cbc_control_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	//if (f54->query.has_ctrl88 == 1) {
		...
		f54->control.reg_88->cbc_polarity = 1;
		f54->control.reg_88->cbc_tx_carrier_selection = 1;
		retval = f54->fn_ptr->write(rmi4_data,
				f54->control.reg_88->address,
				f54->control.reg_88->data,
				sizeof(f54->control.reg_88->data));
...
static ssize_t synaptics_rmi4_f54_cbc_control_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	unsigned char value;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int retval;
	unsigned int setting;
	unsigned char command =  (unsigned char)0x01;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	retval = sstrtoul(buf, 10, &setting);
	if (retval)
		return retval;
	...
	if (setting != 1)
		return -EINVAL;
	mutex_lock(&f54->status_mutex);
    	retval = f54->fn_ptr->write(rmi4_data,rmi4_data->f01_cmd_base_addr,&command,sizeof(command));
	mutex_unlock(&f54->status_mutex);
    	if (retval < 0)
    		return retval;
	msleep(90);
...
static ssize_t synaptics_rmi4_f54_irq_enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (setting == 0)
      {
            retval = f54->fn_ptr->enable(rmi4_data,false);
      }
      else
      {
            retval = f54->fn_ptr->enable(rmi4_data,true);
      }
    	if (retval < 0)
    		return retval;
	return count;
...
pdt_done:
	...
	if( FirstTime == true){
		FirstTime = false ;
		for (ii = 0; ii < ARRAY_SIZE(systemattrs); ii++) {
        		retval = sysfs_create_file(&rmi4_data->i2c_client->dev.kobj,&systemattrs[ii].attr);
        		if (retval < 0) {
				dev_err(&rmi4_data->i2c_client->dev,
        					"%s: Failed to create sysfs attributes\n",
        					__func__);
