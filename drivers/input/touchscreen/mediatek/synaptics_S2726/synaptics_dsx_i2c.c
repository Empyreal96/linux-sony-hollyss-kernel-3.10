...
static unsigned char uctrl_09_offset = 0;
static unsigned char uctrl_20_offset = 0; //
static unsigned char uctrl_23_offset = 0;
static unsigned char uctrl_26_offset = 0;
static unsigned char uf12_ctrl_base_addr = 0;
static unsigned int glovemode = 0;
// extern function
...
static ssize_t synaptics_rmi4_update_FW_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static int synaptics_rmi4_restore_glove_mode(struct synaptics_rmi4_data *rmi4_data); //PROD-DH-TOUCH-GLOVE-00+
...
struct synaptics_rmi4_f12_ctrl_09 {
	union {
		struct {
			unsigned char z_tuning_0;
			unsigned char z_tuning_1;
			unsigned char z_tuning_2;
			unsigned char z_tuning_3;
			unsigned char z_tuning_4;
			unsigned char z_tuning_5;
			unsigned char z_tuning_6;
			unsigned char W_tuning_0;
			unsigned char w_tuning_1;
			unsigned char w_tuning_2;
			unsigned char w_tuning_3;
			unsigned char finger_size_0;
			unsigned char finger_size_1;
			unsigned char finger_size_2;
			unsigned char finger_size_3;
			unsigned char threshold_0;
			unsigned char threshold_1;
			unsigned char threshold_2;
			unsigned char threshold_3;
			unsigned char gloved_detection;
			unsigned char gloved_suppression;
			unsigned char gloved_detection_0;
			unsigned char gloved_detection_1;
			unsigned char gloved_detection_2;
			unsigned char gloved_detection_3;
		};
...
struct synaptics_rmi4_f12_ctrl_23 {
	union {
		struct {
			...
			unsigned char report_as_finger;
		};
		unsigned char data[3];
	};
};
...
struct synaptics_rmi4_f12_ctrl_26 {
	union {
		struct {
			unsigned char glove_finger_detection:1;
			unsigned char closed_cover_operation:1;
			unsigned char fast_glove_detect:1;
...
static struct device_attribute attrs[] = {
	...
	__ATTR(updateglove, (S_IRUGO | S_IWUGO),
			synaptics_rmi4_update_FW_glove_show,
			synaptics_rmi4_store_error),
};
		...
		struct device_attribute *attr, char *buf)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	struct synaptics_rmi4_f12_ctrl_09 ctrl_09;
	struct synaptics_rmi4_f12_ctrl_23 ctrl_23;
	struct synaptics_rmi4_f12_ctrl_26 ctrl_26;

	retval = synaptics_rmi4_i2c_read(rmi4_data,
			uf12_ctrl_base_addr + uctrl_09_offset,
			ctrl_09.data,
			sizeof(ctrl_09.data));
	if (retval < 0)
		return retval;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			uf12_ctrl_base_addr + uctrl_23_offset,
			ctrl_23.data,
			sizeof(ctrl_23.data));
	if (retval < 0)
		return retval;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			uf12_ctrl_base_addr + uctrl_26_offset,
			ctrl_26.data,
			sizeof(ctrl_26.data));
	if (retval < 0)
		return retval;

	//return snprintf(buf, PAGE_SIZE, "%d\n F12_2D_Ctrl09_07  0x%02x \n F12_2D_Ctrl23_00  0x%02x \n F12_2D_Ctrl23_02  0x%02x \n F12_2D_Ctrl26_00  0x%02x \n"
	//,glovemode,ctrl_09.gloved_detection,ctrl_23.obj_type_enable,ctrl_23.report_as_finger,ctrl_26.data[0]);

	return snprintf(buf, PAGE_SIZE, "%d\n F12_2D_Ctrl09_07  0x%04x  0x%02x \n F12_2D_Ctrl23_00  0x%04x  0x%02x \n F12_2D_Ctrl23_02  0x%04x  0x%02x \n F12_2D_Ctrl26_00  0x%04x  0x%02x \n"
	,glovemode
	,uf12_ctrl_base_addr + uctrl_09_offset,ctrl_09.gloved_detection
	,uf12_ctrl_base_addr + uctrl_23_offset,ctrl_23.obj_type_enable
	,uf12_ctrl_base_addr + uctrl_23_offset,ctrl_23.report_as_finger
	,uf12_ctrl_base_addr + uctrl_26_offset,ctrl_26.data[0]);
}
...
*/
	...
	if( glovemode == 1 )
	{
		//ctrl_09.gloved_detection |= 1 ;
		ctrl_23.obj_type_enable |= (1 << 5) ;  //0010 0000
		ctrl_23.report_as_finger &= 0xDF ; //1101 1111
		ctrl_26.glove_finger_detection |= 1 ;
	}
	else
	{
		//ctrl_09.gloved_detection &= 0 ;
		ctrl_23.obj_type_enable &= 0xDF ; //1101 1111
		ctrl_23.report_as_finger |= (1 << 5) ; //0010 0000
		//ctrl_23.report_as_finger &= 0xDF ; //1101 1111
		ctrl_26.glove_finger_detection &= 0 ;
	}
...
exit:
        if(gprDMABuf_va){
                dma_free_coherent(&rmi4_data->i2c_client->dev, 4096, gprDMABuf_va, gprDMABuf_pa);
                gprDMABuf_va = NULL;
                gprDMABuf_pa = NULL;
        }
...
exit:
	...
	if(gpwDMABuf_va){
                dma_free_coherent(&rmi4_data->i2c_client->dev, 1024, gpwDMABuf_va, gpwDMABuf_pa);
                gpwDMABuf_va = NULL;
                gpwDMABuf_pa = NULL;
        }
	mutex_unlock(&(rmi4_data->rmi4_io_ctrl_mutex));
...
extern bool mdss_display_power_state(void);	/* PERI-JC-TOUCH_WORKAROUND-00+ */
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		//printk("ETUCH :object_type_and_status 0x%02x \n",  finger_data->object_type_and_status);
		if( (finger_data->object_type_and_status & MASK_1BIT) || (finger_data->object_type_and_status & 0x06) )
		{
			finger_status = 1;
		}
		else
		...
		static struct pointer_information	pointer_state[] = {
		{false, false, 0 , 0, 0, 0, 0, 0, 0 },
		{false, false, 0 , 0, 0, 0, 0, 0, 0 },
		{false, false, 0 , 0, 0, 0, 0, 0, 0 },
		{false, false, 0 , 0, 0, 0, 0, 0, 0 },
		{false, false, 0 , 0, 0, 0, 0, 0, 0 } };
		struct pointer_information	*finger_state;
		if( finger < sizeof( pointer_state ) / sizeof( *pointer_state ) )
		{
			finger_state = pointer_state + finger;
			...
			{
				finger_state->skip=false;
				finger_state->x=123456;
				finger_state->y=123456;
				finger_state->z=0;
				finger_state->wx=0;
				finger_state->wy=0;
				finger_state->avgx=0;
				finger_state->avgy=0;
				printk("ETUCH : <%d>(%s)[%d:%d:%d][%d:%d]%d\n", finger, finger_state->down_state ? "down" : "up", x , y , finger_data->z,finger_data->wx,finger_data->wy,finger_data->object_type_and_status);
			}
		}
		...
		if (finger_status) {
			...
			{
				if(finger_state->x==123456 && finger_state->y==123456 )
				{
					finger_state->skip=false;
				}
				else
				{
					if(finger_state->avgx-x>3||finger_state->avgx-x<-3||finger_state->avgy-y>3||finger_state->avgy-y<-3)
					{
						finger_state->skip=false;
					}
					else if(finger_state->avgx-finger_state->x>3||finger_state->avgx-finger_state->x<-3||finger_state->avgy-finger_state->y>3||finger_state->avgy-finger_state->y<-3)
					{
						finger_state->skip=false;
					}
					else
					{
						finger_state->skip=true;
					}
				}
				if(finger_state->skip==false)
				{
					finger_state->avgx=x;
					finger_state->avgy=y;
					finger_state->x=x;
					finger_state->y=y;
					finger_state->z=finger_data->z;
					finger_state->wx=wx;
					finger_state->wy=wy;
				}
				else
				{
					finger_state->skip=false;
					finger_state->avgx=(x+finger_state->avgx)/2;
					finger_state->avgy=(y+finger_state->avgy)/2;
					x=finger_state->x;
					y=finger_state->y;
					if(finger_data->z != 150)
					{
						finger_data->z=finger_state->z;
					}
					wx=finger_state->wx;
					wy=finger_state->wy;
				}
			...
			input_report_abs(rmi4_data->input_dev,
					ABS_MT_TOUCH_MINOR, min(wx, wy));
#endif
			//input_report_abs(rmi4_data->input_dev, ABS_MT_PRESSURE, finger_data->z <= 5 ? 255 + finger_data->z + 1 : finger_data->z );
			
			if( finger_data->object_type_and_status & 0x06 )
			{
				input_report_abs(rmi4_data->input_dev, ABS_MT_PRESSURE,  255 + 5 );				
			}
			else
			{
				if(wx == 5 && wy == 5 && finger_data->z == 150)
				{
					input_report_abs(rmi4_data->input_dev, ABS_MT_PRESSURE,  255 + 5 );
				}
				else
				{
					input_report_abs(rmi4_data->input_dev, ABS_MT_PRESSURE,  finger_data->z );
				}
			}
		...
		bool enable)
{
	...
	if (enable) {
		...
		if (!rmi4_data->irq_enabled) {
			...
			//mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_POLARITY_LOW, tpd_eint_handler, 0);
		...
		unsigned int intr_count)
{
	...
	uctrl_09_offset = ctrl_8_offset +
			query_5.ctrl8_is_present ;
	uctrl_23_offset = ctrl_23_offset ;
	uctrl_26_offset = ctrl_23_offset +
			query_5.ctrl23_is_present +
			query_5.ctrl24_is_present +
			query_5.ctrl25_is_present ;
	uf12_ctrl_base_addr = fhandler->full_addr.ctrl_base ;
	...
	printk("%s: ctrl_23_base_addr 0x%04x \n" , __func__, fhandler->full_addr.ctrl_base + ctrl_23_offset );
	/* Maximum number of fingers supported */
	...
	printk("%s: ctrl_8_base_addr 0x%04x \n" , __func__, fhandler->full_addr.ctrl_base + ctrl_8_offset );
	/* Maximum x and y */
	rmi4_data->sensor_max_x =
...
flash_prog_mode:
	...
	printk("Synaptic product id %s\n", rmi->product_id_string );/* PERI-JC-TOUCH_FIRMWARE_RETRY-00+ */
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	/* PERI-JC-TOUCH_FIRMWARE_RETRY-00+[ */
	if(rmi4_data->sensor_max_x==0)
	{
		printk("%s: RTUCH sensor_max_x %d \n" , __func__, rmi4_data->sensor_max_x );
	...
	{
		printk("%s: RTUCH max_touch_width %d \n" , __func__, rmi4_data->max_touch_width );
		rmi4_data->max_touch_width = 28;
	}
	/* PERI-JC-TOUCH_FIRMWARE_RETRY-00+] */
	input_set_abs_params(rmi4_data->input_dev,
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	synaptics_rmi4_restore_glove_mode(rmi4_data); //PROD-DH-TOUCH-GLOVE-00+
	return 0;
}
...
/* PERI-JC-TOUCH_PING_HW-00+[ */
static int synaptics_rmi4_ping_hw(struct synaptics_rmi4_data *rmi4_data)
{
	int retval ;
	unsigned char data;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,0x00,(unsigned char *)&data,sizeof(data));
	return retval;
}
/* PERI-JC-TOUCH_PING_HW-00+] */
//PROD-DH-TOUCH-GLOVE-00+[
static int synaptics_rmi4_restore_glove_mode(struct synaptics_rmi4_data *rmi4_data)
{
	int retval ;
...
{
	int retval;
	unsigned char attr_count,i2c_attr_count;
	struct synaptics_rmi4_data *rmi4_data;
...
/* PERI-JC-TOUCH_PING_HW-00+[ */
	retval = synaptics_rmi4_ping_hw(rmi4_data);
	if (retval < 0) {
		dev_err(&client->dev,"%s: No synaptics hw detected\n",__func__);
...
/* PERI-JC-TOUCH_PING_HW-00+] */
	...
	for (i2c_attr_count = 0; i2c_attr_count < ARRAY_SIZE(attrs); i2c_attr_count++) {
		retval = sysfs_create_file(&rmi4_data->i2c_client->dev.kobj,
				&attrs[i2c_attr_count].attr);
		if (retval < 0) {
			dev_err(&client->dev,
					"%s: Failed to create sysfs attributes\n",
					__func__);
	...
	tpd_hw = 1 ; /* PERI-JC-TOUCH_PING_HW-00+ */
	tpd_load_status = 1;
	g_dev = &rmi4_data->input_dev->dev;
	return retval;

i2c_err_sysfs:
	for (i2c_attr_count--; i2c_attr_count >= 0; i2c_attr_count--) {
...
{
	printk("MTKTOUCH tpd_local_init driver \n");
...
static int __init synaptics_rmi4_init(void)
{
	...
	i2c_register_board_info(TPD_I2C_BUS_ID, &i2c_tpd, 1);
