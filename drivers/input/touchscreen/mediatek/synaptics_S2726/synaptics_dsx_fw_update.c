...
/* PERI-JC-TOUCH_CHECK_CONFIG-01+[ */
#define SYNAPTICS_CONFIG_CHECK

#ifdef SYNAPTICS_CONFIG_CHECK
#define SYNAPTICS_FIRMWARE_VERSION_DP   1804153
#define SYNAPTICS_CONFIG_VERSION_DP   0x00010001
#define SYNAPTICS_CONFIG_VERSION_SP   0x00030002
#endif
/* PERI-JC-TOUCH_CHECK_CONFIG-01+] */
...
static struct device_attribute myattrs[] = {
...
/* PERI-JC-TOUCH_CHECK_FIRMWARE-00+[ */
/*#ifdef SYNAPTICS_CONFIG_CHECK
	if (image_fw_id >= SYNAPTICS_FIRMWARE_VERSION_DP && (fih_get_product_phase() == PHASE_EVM||fih_get_product_phase() == PHASE_PD||fih_get_product_phase() == PHASE_DP))
	{
		flash_area = NONE;
		goto exit;
	}
...
/* PERI-JC-TOUCH_CHECK_CONFIG-01+[ */
/*#ifdef SYNAPTICS_CONFIG_CHECK
	if (image_config_id >= SYNAPTICS_CONFIG_VERSION_DP && (fih_get_product_phase() == PHASE_EVM||fih_get_product_phase() == PHASE_PD||fih_get_product_phase() == PHASE_DP))
	{
		flash_area = NONE;
		goto exit;
	}
	else if (image_config_id > SYNAPTICS_CONFIG_VERSION_SP && (fih_get_product_phase() == PHASE_SP||fih_get_product_phase() == PHASE_SP_AP1))
	{
		flash_area = NONE;
		goto exit;
	}
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	for( count = 0 ; count < 3 ; ++count)
	{
		...
		if( retval < 0 )
		{
			printk("%s: RTUCH retry count %d , %d \n" , __func__,count, retval );
			msleep(1000);
		}
		else
		{
...
{
	int retval;
	unsigned char attr_count , myattr_count;
	...
	for (myattr_count = 0; myattr_count < ARRAY_SIZE(myattrs); myattr_count++) {
		retval = sysfs_create_file(&rmi4_data->i2c_client->dev.kobj,
				&myattrs[myattr_count].attr);
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev,
					"%s: Failed to create sysfs attributes\n",
					__func__);
			retval = -ENODEV;
			goto exit_remove_myattrs;
		}
	}
	...
	/* PERI-JC-TOUCH_FIRMWARE_RETRY-00+[ */
	if(strcmp(rmi4_data->rmi4_mod_info.product_id_string,"s2726")==0 && rmi4_data->flash_prog_mode == true)
	{
	}
	else
	{
		if( fwu->rmi4_data->firmware_id == 1634853 )//M812 module
		    return 0;
	}
	/* PERI-JC-TOUCH_FIRMWARE_RETRY-00+] */
...
exit_remove_myattrs:
for (myattr_count--; myattr_count >= 0; myattr_count--) {
...
static void synaptics_rmi4_fwu_remove(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(myattrs); attr_count++) {
        	sysfs_remove_file(&rmi4_data->i2c_client->dev.kobj,
        			&myattrs[attr_count].attr);
        }
	kfree(fwu->read_config_buf);
	kfree(fwu->image_name);
	kfree(fwu->fn_ptr);
