...
static int mmc_device_power_control_ioctl(struct block_device *bdev,
	struct mmc_ioc_device_power_control  __user *ic_ptr)
{
	struct mmc_ioc_device_power_control *idata;
	struct mmc_blk_data *md;
	struct mmc_card *card;
	int err=0;
	...
	if (copy_from_user(idata, ic_ptr, sizeof(*idata))) {
		err = -EFAULT;
		goto out;
	}
	...
	/*eMMC cannot power control*/
	if (mmc_card_mmc(card)){
		pr_err("[SD_DEBUG]eMMC cannot power control\n");
		err = -EINVAL;
		goto cmd_done;
	}
	...
	err = card->host->ops->device_power_control(card->host, idata->flag);
	if (err == MMC_POWER_CONTROL_FAIL) {
		pr_err("[SD_DEBUG]Power Control: %s: error %d POWER %s Faild, because of other reasons\n",
			mmc_hostname(card->host), err, idata->flag == MMC_DEVICE_POWER_ON ? "ON" : "OFF");
	}else if (err == MMC_POWER_CONTROL_OVER_CURRENT) {
		pr_err("[SD_DEBUG]Power Control: %s: error %d POWER %s Faild, because of over-current sd\n",
			mmc_hostname(card->host), err, idata->flag == MMC_DEVICE_POWER_ON ? "ON" : "OFF");
	}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	else if (cmd == MMC_IOC_DEVICE_POWER_CONTROL) {
		pr_err("[SD_DEBUG] receive ioctl for power control\n");
		ret = mmc_device_power_control_ioctl(bdev, (struct mmc_ioc_device_power_control __user *)arg);
	}
